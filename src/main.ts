...
new engine.Engine(<HTMLCanvasElement>document.getElementById("canvas"), (delta: number, fps: number, engineInstance: engine.Engine) => {
  ...
  engineInstance.DrawTexturedRectangle(100, 100, "arrow", {
  ...
  engineInstance.DrawTexturedRectangle(200, 200, "google", {
    matrix: math.multiply(gt, matrix),
  });
  //return;
  ...
  engineInstance.DrawTexturedRectangle(sx + Math.sin(date_now / speedx) * ampx, sy + Math.sin(date_now / speedy) * ampy, "starbucks", {
    matrix: math.multiply(gt, matrix),
  });
  ...
  120 * 3,
  2,
  { arrow, starbucks, gradient, google }
);